...
class CNFGrammar:
    ...
    def __init__(self, rules, start):
        ...
        self._start = start
        self._steps_to_null = None  # steps_to_null

        self._term_to_symbol_steps = collections.defaultdict(list)
        self._symbol_pair_to_source_steps = collections.defaultdict(list)
        symbol_pair_to_source = collections.defaultdict(list)

        for lhs, options in rules.items():
            for rhs, steps in options:
                if isinstance(rhs, str):
                    self._term_to_symbol_steps[rhs].append((lhs, steps))
                else:
                    self._symbol_pair_to_source_steps[rhs].append((lhs, steps))
                    symbol_pair_to_source[rhs].append(lhs)

        self._term_to_symbol_steps = {key: tuple(sorted(value))
                                       for key, value
                                       in self._term_to_symbol_steps.items()}

        self._symbol_pair_to_source_steps = {key: tuple(sorted(value))
                                              for key, value
                                              in self._symbol_pair_to_source_steps.items()}
            ...
            return tuple(set(filter(None,
                                    map(symbol_pair_to_source.get,
                                        itertools.product(left_cands,
                                                          right_cands)))))
        self._sources_to_generate_span = sources_to_generate_span

    def matches(self, message):
        # https://en.wikipedia.org/wiki/CYK_algorithm
        if len(message) == 0:
            return self._steps_to_null is not None

        P = [[tuple()] * (len(message)-i+1+1) for i in range(len(message)+1)]
        for idx, term in enumerate(message):
            s = idx+1
            symbol_steps = self._term_to_symbol_steps.get(term)
        ...
        for l in range(2, len(message)+1): # Length of span
            for s in range(1, len(message)-l+2): # Start of span
                source_lists = [self._sources_to_generate_span(P[p][s],
                                                                P[l-p][s+p])
            ...
            for s in range(1, len(message)-l+2): # Start of span
                ...
                for p in range(1, l): # Partition of span
                    for (l_s, l_steps), (r_s, r_steps) in itertools.product(P[p][s],
                                                                            P[l-p][s+p]):
                        symbol_steps = self._symbol_pair_to_source_steps.get((l_s, r_s))
                        if symbol_steps is None:
                            continue
...
def to_grammar(rules):
    ...
    for base, options in sorted(rules.items()):
        ...
        for opt in options:
            ...
            yield Symbol(base), list(map(Symbol,
                                                 option_elements))

    for elem in known_elements:
        # Mark this free, we only use symbols for compatibility with lib.cyk!
        yield Symbol(elem), elem, 0

def part2(s):
    rules, molecule = parse_input(s)

    cnf = CNFGrammar(to_grammar(rules), Symbol('e'))
